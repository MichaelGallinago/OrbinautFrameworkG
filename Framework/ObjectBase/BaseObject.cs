	...
	public void ActSolid(Player player, Constants.SolidType type)
	{
		...
		else if (type != Constants.SolidType.Top)
		{
			...
			bool vCollision = Math.Abs(clip.X) >= Math.Abs(clip.Y) || 
			    SharedData.PlayerPhysics == PhysicsTypes.SK && Math.Abs(clip.Y) <= 4;
				...
				switch (clip.Y)
				{
					...
					case < 0 when type != Constants.SolidType.ItemBox:
						switch (player.Speed.Y)
						{
							...
							case < 0:
								if (SharedData.PlayerPhysics >= PhysicsTypes.S3 && !player.IsGrounded)
								{
			...
			if (!(SharedData.PlayerPhysics == PhysicsTypes.SK || Math.Abs(clip.Y) > 4))
			{
				player.ClearPush();
				return;
			}
